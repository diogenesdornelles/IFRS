...
function FormBook(): JSX.Element {
  const validationSchema = Yup.object().shape({
    ...
    publishDate: Yup.string().required("A data de publicação é obrigatória."),
  });
  ...
  useEffect(() => {
    if (_id) {
      ...
      if (bookRef.current) {
        ...
        bookRef.current.publishDate = bookRef.current.publishDate.slice(
          0,
          10
        );
    ...
    async function processReq() {
      ...
      if (bookResponse) {
        ...
        if (file) {
          ...
          const fileResponse = await handler.executeRequest({
            operation: "FILE",
            formData,
        ...
        setBooks((prevState) => {
          if (isFormNewBook) {
            return [...prevState, fileResponse ? fileResponse : bookResponse];
          } else {
            return prevState.map((book: TBook) => {
              return book._id === (fileResponse?._id || bookResponse._id)
                ? fileResponse
                  ? fileResponse
                  : bookResponse
                : book;
  ...
  return (
    <>
      ...
      <div className="flex flex-col justify-center m-auto backdrop-blur-sm max-w-[800px] bg-black/50 rounded-lg text-white text-sm p-6 animate__animated animate__fadeIn mt-20 mb-52">
        ...
        >
          {({ errors, touched }) => (
            <Form>
              <div className="mb-4">
                <label htmlFor="file_input" className="block text-white mb-2">
                ...
                <input
                  ...
                  name="file"
                  type="file"
                  className="block w-full text-gray-900 border border-gray-300 rounded-lg bg-gray-50 focus:ring focus:ring-blue-500"
                />
                {!isValidFile.valid && (
              ...
              {[
                ...
                <div className="mb-4" key={field}>
                  ...
                  <Field
                    ...
                    type={type}
                    name={field}
                    className={`w-full text-gray-900 border border-gray-300 rounded-lg p-2.5 ${
              ...
              <button
                type="submit"
                className="btn bg-lime-800 hover:bg-lime-900 text-white w-full py-4 rounded-lg mb-4 mt-10"
              >
                {isFormNewBook ? "Criar" : "Atualizar"}
              </button>
            </Form>
          )}
        </Formik>
