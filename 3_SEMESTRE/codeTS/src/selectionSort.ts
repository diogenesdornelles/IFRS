...
function selectionSort(arr: number[]): number[] {
  ...
  for (let i = 0; i < n - 1; i++) {
    // cria uma primeira iteração, que percorre o array de 0 até o penúltimo
    let minIdx = i; // Assume que o menor elemento é o atual
    // Itera sobre a parte não ordenada do array, de i a arr.length
    for (let j = i + 1; j < n; j++) {
      // cria uma segunda iteração, que percorre de n + 1 até o fim do array. É a parte não ordenada.
      // Se encontrar um elemento menor que o atual mínimo, atualiza o índice do mínimo
      if (arr[j] < arr[minIdx]) {
        // se o valor da parte ordenada for maior, então
    ...
    if (minIdx !== i) {
      // se os índices forem diferentes, então troca
